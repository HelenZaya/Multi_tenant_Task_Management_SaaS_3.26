...
export const authRepository = {
  ...
  async createTenantWithAdmin(data: { name: string; email: string; passwordHash: string; tenantName: string; tenantSlug: string }) {
  ...
  revokeRefreshToken(jti: string, replacedById?: string) {
    return prisma.refreshToken.update({
      where: { jti },
      data: { revokedAt: new Date(), replacedById }
    });
  },
  writeAuditLog(tenantId: string, actorId: string | null, action: string, entityType: string, entityId: string, payload?: Prisma.InputJsonValue) {
    return prisma.auditLog.create({
      data: { tenantId, actorId: actorId ?? undefined, action, entityType, entityId, payload }
    });
  },
  createInvitationMembership(data: { tenantId: string; userId: string; role: Role; invitedById: string }) {
