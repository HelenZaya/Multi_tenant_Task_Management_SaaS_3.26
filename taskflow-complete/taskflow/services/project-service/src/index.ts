...
const env = loadEnv();
const logger = createLogger("project-service", env.LOG_LEVEL);
...
async function main() {
  ...
  await startServer(app, env.PROJECT_SERVICE_PORT, logger);
...
main().catch((err) => {
  logger.fatal({ err }, "Failed to start project-service");
  process.exit(1);
});
